...
## 1) Rail Map (what each pin wants)
...
- **`VDDRF` → `+3V3_SYS` (same net as VDD)**  
  Dedicated RF supply **tied directly** to VDD (no bead). Add **100 nF at the pin**.
...
- **`VFBSMPS`**: SMPS output node. Place **Cbulk = 4.7 µF** from `VFBSMPS` → `GND` close to pins.  
  **Do not use `VFBSMPS` to power external loads**; it varies with RF/TX state.
...
## 2) Ground Strategy (single, solid plane)
...
## 4) Net Labels & Classes (to drive layout rules later)
...
## 6) Starter BOM Hints (0402 unless noted)
