...
- Bus labels: `I2C_SCL`, `I2C_SDA`, `BMI270_INT1`, `BMI270_INT2`, `BTN_IN`, `GPIO_LED`, `SENS_EN`.
...
**Purpose:** Combine the sensors (BMI270, SHTC3, LPS22HH) with the user button and status LED.  
...
- Start @ 100 kHz, analog filter on, digital filter = 0.

### Sensors & addresses (no conflicts)
- **BMI270 (IMU):** I²C, **0x68** (`SDO → GND`); `INT1→PA0`, `INT2→PA1` (EXTI rising).  
- **SHTC3 (T/RH):** fixed **0x70**.  
- **LPS22HH (Baro):** **0x5C** (`SA0 → GND`) or **0x5D** (`SA0 → VDD_SENS`). Choose one and label.

> Keep this sheet physically **outside the antenna keepout**; put SHTC3/LPS22HH near a vent/slot.
...
### Net labels (copy into AD25)
```
VDD_SENS, +3V3_SYS, GND
I2C_SCL, I2C_SDA, BMI270_INT1, BMI270_INT2
BTN_IN, GPIO_LED
...
- Mark LPS22HH port keepout; SHTC3 vent path; button finger clearance.  
...
2) `SENS_EN=High` → `VDD_SENS≈3.3 V` at TP; I²C scan = **0x68/0x5C(or 0x5D)/0x70**.  
3) LED/SW sanity: blink LED; EXTI falling on button.  
4) Raise I²C to 400 kHz; enable BMI270 FIFO/DRDY; LPS22HH 25 Hz; SHTC3 1 Hz. Observe current delta with sensors on vs off.
...
- **Sensor decoupling:** BMI270 0.1 µF + 1 µF; SHTC3 0.1 µF; **LPS22HH 0.1 µF + 1 µF**.
...
- **Sensor_IO_Buttons_LED.SchDoc**
  ...
  - `BTN_IN → PB1` (to GND on press), `GPIO_LED → PB0` (choose active‑low/high option), `BMI270_INT1/2 → PA0/PA1`.
